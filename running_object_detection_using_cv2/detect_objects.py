...
def DetectFromVideo(detector, Video_path, save_output=False, output_dir='output/', isShow=True):
...
def DetectImagesFromFolder(detector, images_dir, save_output=False, output_dir='output/', isShow=True):
...
if __name__ == "__main__":
	...
	parser.add_argument('--images_dir', help='Directory to input images)', default='data/test_images/')
	parser.add_argument('--video_path', help='Path to input video)', default='data/samples/pedestrian_test.mp4')
	parser.add_argument('--output_directory', help='Path to output images and video', default='/media/nourislam/Data/CodeForCourses/ObjectDetection/Experiment_1/inference/data/output/')
	...
	parser.add_argument('--isShow', help='To show detected images or videos, default: False',
						action='store_true')
	...
	if args.video_input:
		DetectFromVideo(detector, args.video_path, save_output=args.save_output, output_dir=args.output_directory, isShow=args.isShow)
	else:
		DetectImagesFromFolder(detector, args.images_dir, save_output=args.save_output, output_dir=args.output_directory, isShow=args.isShow)
